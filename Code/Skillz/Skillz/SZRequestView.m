...
- (id)initWithRequest:(SZEntryVO*)request
{
    ...
    if (self) {
		...
		if (self.request.hasTimeFrame) [self addSubview:[self timeFrameSection]];
...
- (SZTitleView*)titleSection {
	...
	if (_titleSection == nil) {
		NSString* categoriesString = self.request.category;
		if (self.request.subcategory != nil) {
			categoriesString = [categoriesString stringByAppendingFormat:@" > %@", self.request.subcategory];
		}
		
		_titleSection = [[SZTitleView alloc] initWithTitle:self.request.title category:categoriesString];
...
- (SZTimeFrameView*)timeFrameSection {
	if (_timeFrameSection == nil) {
		_timeFrameSection = [[SZTimeFrameView alloc] initWithStartDate:self.request.startTime endDate:self.request.endTime];
...
- (SZUserAreaView*)userSection {
	if (_userSection == nil) {
		_userSection = [[SZUserAreaView alloc] initWithUser:nil hasTimeFrameView:self.request.hasTimeFrame];
...
- (SZDescriptionView*)descriptionSection {
	if (_descriptionSection == nil) {
		_descriptionSection = [[SZDescriptionView alloc] initWithDescription:self.request.description];
...
- (UIView*)priceView {
	...
	if (_priceView == nil) {
		...
		if (self.request.priceIsFixedPerHour || self.request.priceIsFixedPerJob) {
			...
			[pointsLabel setText:[self.request.price stringValue]];
			...
			[perLabel setText:self.request.priceIsFixedPerHour ? @"Skillpoints\nper hour" : @"Skillpoints\nper job"];
...
- (UIView*)locationView {
	if (_locationView == nil) {
		...
		if (self.request.locationIsRemote) {
		...
		else if (self.request.locationWillGoSomewhere) {
			if (self.request.withinZipCode) {
				if ([[SZDataManager sharedInstance].currentUser.zipCode isEqualToString:self.request.user.zipCode]) { // xy will come to you
					UILabel* label1 = [self semiBoldGrayLabelWithSize:15.0];
					[label1 setFrame:CGRectMake(0.0, 8.0, _locationView.frame.size.width, 20.0)];
					[label1 setText:self.request.user.firstName];
					...
					UILabel* label2 = [self semiBoldGrayLabelWithSize:15.0];
					[label2 setFrame:CGRectMake(0.0, 28.0, _locationView.frame.size.width, 20.0)];
					[label2 setText:[NSString stringWithFormat:@"%@'s area.", self.request.user.firstName]];
					[_locationView addSubview:label2];
			...
			else if (self.request.withinSpecifiedArea) {
				if ([SZDataManager sharedInstance].currentUser.hasFullAddress) {
					if ([[SZDataManager sharedInstance].currentUser isWithinDistance:self.request.distance ofAddress:self.request.address]) { // xy will come to you
						UILabel* label1 = [self semiBoldGrayLabelWithSize:15.0];
						[label1 setFrame:CGRectMake(0.0, 8.0, _locationView.frame.size.width, 20.0)];
						[label1 setText:self.request.user.firstName];
						[_locationView addSubview:label1];
			...
			else if (self.request.withinNegotiableArea) { // xy might come to you
				UILabel* label1 = [self semiBoldGrayLabelWithSize:15.0];
			...
			UILabel* label2 = [self semiBoldGrayLabelWithSize:15.0];
			[label2 setFrame:CGRectMake(0.0, 28.0, _locationView.frame.size.width, 20.0)];
			[label2 setText:[NSString stringWithFormat:@"%@'s.", self.request.user.firstName]];
			[_locationView addSubview:label2];
