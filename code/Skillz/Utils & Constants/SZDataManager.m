...
//	[[NSUserDefaults standardUserDefaults] setObject:[NSDictionary dictionary] forKey:MESSAGES];
	...
	if (![reach isReachable]) {	// network not available
		...
		// query for all messages received after the last cached message
		...
		[messageQuery findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
			if (objects && [objects count] > 0) {
				...
				for (PFObject* message in objects) {
					...
					// when a message is saved successfully, the store count will be incremented
					[SZDataManager addMessageToUserCache:message completionBlock:^(BOOL finished) {
						storeCount++;
						NSLog(@"stored message %i", storeCount);
						// dispath the completion block after all messages are saved
	...
	if (![fromUser isEqual:[PFUser currentUser]]) {
		[fromUser fetchIfNeededInBackgroundWithBlock:^(PFObject *object, NSError *error) {
			if (object) {
				...
				fromUserFetched = YES;
				// the block will be only dispatched once, depending on which fetch operation is finished last
...
+ (NSMutableArray*)getGroupedMessages {
	...
	for (NSString* key in messageKeys) {
		...
		NSString* userID; // the user ID will be used as the key for the message thread dictionary, cause we're grouping by user
		...
		if ([messagesGrouped valueForKey:userID]) {
			// if the userID key already exists, we're retrieving the existing array for the message thread
			messageArray = [messagesGrouped valueForKey:userID];
		}
		else {
			// if the userID key doesn't exist yet, we're creating a new array for it and storing it in the dictionary
			messageArray = [[NSMutableArray alloc] init];
			[messagesGrouped setObject:messageArray forKey:userID];
			
		}
		// adds the message to the correct message thread array
...
+ (void)addOpenDealToUserCache:(PFObject*)openDeal {
	...
	else entryFetchted = YES;
	
	
	if ([[[openDeal valueForKey:FROM_USER] objectId] isEqual:[[PFUser currentUser] objectId]]) {
		// we're proposing the deal to someone else
		...
		[toUser fetchIfNeededInBackgroundWithBlock:^(PFObject *object, NSError *error) {
				if (object) {
					...
					// the deal will only be saved once, depending on which thread is finished first
					if (entryFetchted) {
						[openDealsArr addObject:deal];
						[[NSUserDefaults standardUserDefaults] setObject:openDealsArr forKey:OPEN_DEALS];
