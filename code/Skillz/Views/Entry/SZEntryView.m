...
- (UIView*)locationView {
	if (_locationView == nil) {
		...
		// location view will display different text depending on where the deal is located and where the user is located
		...
		if (self.entry.locationType == SZEntryLocationRemote) {
			...
			if (self.entry.areaType == SZEntryAreaWithinZipCode) {
				// if the user is in the same zip code area as the entry poster
				...
				// if the user is outside of the entry's zip code area
				else {
				...
				if ([SZDataManager sharedInstance].searchLocationBase) {
					
					// if the user is within the specifed distance of the entry
					if ([[SZDataManager sharedInstance].searchLocationBase objectForKey:@"geoPoint"] && [[[SZDataManager sharedInstance].searchLocationBase objectForKey:@"geoPoint"] distanceInMilesTo:self.entry.geoPoint] <= [self.entry.distance doubleValue]) {  // xy will come to you
						UILabel* label1 = [self semiBoldGrayLabelWithSize:15.0];
						[label1 setFrame:CGRectMake(0.0, 13.0, _locationView.frame.size.width, 20.0)];
						[label1 setText:[self.entry.user valueForKey:@"firstName"]];
						[_locationView addSubview:label1];
					...
					// if the user is NOT within the specifed distance of the entry
					else { // 
						UILabel* label1 = [self semiBoldGrayLabelWithSize:15.0]; // you're outside of xy's area
						[label1 setFrame:CGRectMake(0.0, 13.0, _locationView.frame.size.width, 20.0)];
						[label1 setText:@"You're outside of"];
				...
				// if the user distance can't be determined
				else {
					UILabel* label1 = [self semiBoldGrayLabelWithSize:15.0];
					[label1 setFrame:CGRectMake(0.0, 13.0, _locationView.frame.size.width, 20.0)];
					[label1 setText:[self.entry.user valueForKey:@"firstName"]];
			...
			// if the travel distance of the entry is negotiable
			else if (self.entry.areaType == SZEntryAreaWithinNegotiableArea) { 
				UILabel* label1 = [self semiBoldGrayLabelWithSize:15.0];
		...
		// if the user has to go to where the entry is located
		else { 
			UILabel* label1 = [self semiBoldGrayLabelWithSize:15.0];
			[label1 setFrame:CGRectMake(0.0, 13.0, _locationView.frame.size.width, 20.0)];
