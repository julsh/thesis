	...
	if (self.entry.address || self.entry.areaType == SZEntryAreaWithinZipCode) {
		...
		[geoCoder geocodeAddressString:addressString completionHandler:^(NSArray *placemarks, NSError *error) {
			if (placemarks && [placemarks count] > 0) {
				...
				// if the entry is specified to zip code area or an address & radius, we'll draw a little circle roughly around the area
				...
				// otherwise it'll just be a pin, with no area circle
